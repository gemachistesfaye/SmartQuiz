      ...
      <div className="space-y-5 pb-4">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { to: '/quiz', label: 'Take Quiz', icon: <Zap size={20} />, color: 'bg-yellow-500/10 text-yellow-400 border-yellow-500/20' },
              { to: '/codelab', label: 'Code Lab', icon: <Code size={20} />, color: 'bg-purple-500/10 text-purple-400 border-purple-500/20' },
              { to: '/theory', label: 'Theory', icon: <BookOpen size={20} />, color: 'bg-blue-500/10 text-blue-400 border-blue-500/20' },
              { to: '/ai-assistant', label: 'AI Tutor', icon: <Brain size={20} />, color: 'bg-green-500/10 text-green-400 border-green-500/20' },
            ].map((action) => (
              <Link
                key={action.to}
                to={action.to}
                className={`flex flex-col items-center gap-3 p-4 md:p-5 rounded-2xl border hover:scale-[1.02] transition-all ${action.color}`}
              >
                {action.icon}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            <div className="bg-[#111] border border-white/5 rounded-2xl p-4 md:p-5">
              <h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
                <Zap size={16} className="text-yellow-400" /> Study Streak
              </h3>
              <div className="flex justify-between mb-3">
                {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((day, i) => (
                  <div key={i} className={`w-8 h-8 md:w-9 md:h-9 rounded-full flex items-center justify-center text-[10px] font-bold transition-all ${
              ...
              <p className="text-xs text-gray-400">
                {userData?.streak > 0 ? (
                  <>🔥 <span className="text-white font-bold">{userData.streak}-day streak!</span> Keep it going!</>
                ) : (
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
            <div className="bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/10 rounded-2xl p-4 md:p-5">
              <h3 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
                <Target size={16} className="text-primary" /> Daily Challenge
              </h3>
              <p className="text-xs text-gray-400 mb-4">Complete 3 quizzes today to earn bonus XP</p>
              ...
              <Link to="/quiz" className="block w-full bg-primary hover:bg-primary/90 text-white py-2.5 rounded-xl text-xs font-bold transition-all text-center">
